...
fetchArticleContent url = do
  ...
  hdrs <- getHeaders
  resp <- HTTP.httpBS $ HTTP.setRequestHeaders hdrs req
...
getHeaders :: IO [HTTP.Header]
getHeaders = do
  mCookie <- loadCookieHeader
  return $ maybe baseHeaders (\c -> c : baseHeaders) mCookie

loadCookieHeader :: IO (Maybe HTTP.Header)
loadCookieHeader = do
  mRaw <- lookupEnv "NYTRSS_COOKIE"
  case mRaw of
    Just raw | not (null raw) -> return $ Just ("Cookie", BS.fromString raw)
    _ -> do
      mPath <- lookupEnv "NYTRSS_COOKIE_FILE"
      case mPath of
        ...
        Just path -> do
          ...
          if not exists
            then return Nothing
              ...
              let cookie = parseNetscapeCookies content
              if T.null cookie
                then return Nothing
                else return $ Just ("Cookie", BS.fromString (T.unpack cookie))

parseNetscapeCookies :: Text -> Text
parseNetscapeCookies txt =
  let rows = filter validLine (T.lines txt)
      parts = mapMaybe parseLine rows
  ...
  where
    ...
    parseLine line =
      case T.splitOn "\t" (T.strip line) of
        (domain : _ : _ : _ : _ : name : value : []) ->
          if domainMatch domain then Just (name <> "=" <> value) else Nothing
        _ -> Nothing
    domainMatch domain =
      let d = T.toLower (T.strip domain)
       in "nytimes.com" `T.isSuffixOf` d

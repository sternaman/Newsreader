...
uint8_t CrossPointSettings::writeSettings(FsFile& file, bool count_only) const {
  ...
  // News sync fields (added at end for backward compatibility)
  writer.writeItemString(file, opdsNewsPath);
  writer.writeItem(file, autoNewsSyncOnBoot);
  writer.writeItemString(file, opdsNewsBloombergPath);
  writer.writeItemString(file, opdsNewsBusinessweekPath);
  writer.writeItemString(file, opdsNewsWsjPath);
  writer.writeItemString(file, opdsNewsNytPath);
...
bool CrossPointSettings::saveToFile() const {
  ...
  const uint8_t item_count = writeSettings(outputFile, true);  // This will just count, not write

  // Write header
  serialization::writePod(outputFile, SETTINGS_FILE_VERSION);
  serialization::writePod(outputFile, item_count);
